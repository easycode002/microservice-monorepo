...
class AuthService {
      ...
      if (typeof error === "object" && error !== null && "name" in error) {
        if ((error as { name: string }).name === "NotAuthorizedException") {
          throw new InvalidInputError({
            message: AUTH_MESSAGE.AUTHENTICATION.ACCOUNT_NOT_FOUND,
          });
        }
      }
      ...
      if (typeof error === "object" && error !== null && "name" in error) {
        if ((error as { name: string }).name === "InternalErrorException") {
          throw new InternalServerError({
            message: AUTH_MESSAGE.ERRORS.TECHNICAL_ISSUE,
          });
        }
      }
  ...
  // Method for user login via Facebook
